...
<template>
  <BasicModal
    v-bind="$attrs"
    class="UUIDDom-uuid-modal UUIDDom-uuid-InternalDataSourceModal"
    @register="register"
    :title="titleValue"
    @ok="validateForm"
    @cancel="reset"
    ...
    <div class="uuid-modal-content">
      ...
      <div class="content">
        ...
        <div class="item w-100%!">
          ...
          <div class="item-value">{{ staticValTypeDic[recordValue.staticValType] }}</div>
        </div>
        <div class="item w-100%!">
          <div class="item-title">绑定数据定义根节点UUID：</div>
          <div class="item-value">
            <!-- {{ recordValue.dataDefUUID }} -->
            <UUIDDiv :uuid="recordValue.dataDefUUID" :uuidType="UUIDTypeEnum.InnerDataDefMemSync" />
            <Button
              style="margin-top: -5px; margin-left: -5px"
              title="一键复制"
              size="small"
        ...
        <div class="item w-100%!">
          <div class="item-title">唯一识别字段：</div>
          <div class="item-value">
            <!-- {{ recordValue.idFieldUUID }} -->
            <UUIDDiv :uuid="recordValue.idFieldUUID" :uuidType="UUIDTypeEnum.InnerDataDefMemSync" />
            <Button
              style="margin-top: -5px; margin-left: -5px"
              title="一键复制"
              size="small"
        ...
        <div class="item w-100%!">
          ...
          <div class="item-value">
            {{ recordValue.idFieldValue }}
            <Button
              style="margin-top: -5px; margin-left: -5px"
              title="一键复制"
              size="small"
              type="text"
              @click="oneCopyText(recordValue.idFieldValue)"
            >
              <template #icon><CopyOutlined style="font-size: 12px" /> </template>
            </Button>
          </div>
        ...
        <div class="item">
          <div class="item-title">是否静态：</div>
          <div class="item-value">{{ recordValue.isStatic }}</div>
        </div>
        <div class="item w-100%!">
...
<script lang="ts" setup>
  ...
  import { staticValTypeDic } from '@/utils/commonDic';
  import { UUIDDiv, UUIDModal, uuidModalConfigType, UUIDTypeEnum } from '@/components/UUIDDom';

  defineOptions({ name: 'InternalDataSourceModal' });
...
<style lang="less">
  .UUIDDom-uuid-InternalDataSourceModal {
    // 样式背景覆盖
    &.ant-modal {
      @uuid_bg: './images/InternalDataSourceBg.png';
