...
<template>
  <BasicModal
    v-bind="$attrs"
    class="UUIDDom-uuid-modal UUIDDom-uuid-InnerDataDefineModal"
    @register="register"
    :title="titleValue"
    @ok="validateForm"
    @cancel="reset"
    ...
    <div class="uuid-modal-content">
      ...
      <div class="content">
        ...
        <div class="item w-100%!">
          ...
          <div class="item-value">{{ innerDataTypeBaseDic[recordValue.dataTypeBase] }}</div>
        </div>
        <div class="item w-100%!">
        ...
        <div class="item w-100%!">
          ...
          <div class="item-value">
            <div v-for="(item, index) in recordValue.childUUID" :key="index">
              <!-- <span>{{ item }}</span> -->
              <UUIDDiv v-if="item" :uuid="item" :uuidType="UUIDTypeEnum.InnerDataDefMemSync" />
              <Button
                v-if="item"
                style="margin-top: -5px; margin-left: -5px"
                title="一键复制"
...
<script lang="ts" setup>
  ...
  import { innerDataTypeBaseDic } from '@/utils/commonDic';
...
<style lang="less">
  .UUIDDom-uuid-InnerDataDefineModal {
    // 样式背景覆盖
    &.ant-modal {
      @uuid_bg: './images/InnerDataDefineBg.png';
